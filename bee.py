...
class Bee :
        ...
        for itr in range(self.locIterations):
            while(True):
                ...
                for i in range(len(self.solution.get_state())):
                    ...
                    quality = self.solution.get_mse(self.solution.get_state())
    ...
    def ql_localSearch(self,maxIterIndex,flip):
        ...
        for itr in range(iterations):
          ...
          mse_state = self.solution.get_mse(state)
          mse_new_state = self.solution.get_mse(next_state)

          if (mse_state < mse_new_state):
              reward = mse_new_state
          elif (mse_state > mse_new_state):
              reward = mse_new_state - mse_state
          else :
              if (Solution.nbrUn(state) > Solution.nbrUn(next_state) ):
                  reward = -1/2 * mse_new_state
              else :
                  reward = 1/2 * mse_new_state

          self.data.ql.learn(self.solution,action,reward,next_sol)
          self.rl_return = self.data.ql.get_q_value(self.solution,action)
          self.fitness = mse_new_state
          self.solution = next_sol
          #print("Next state's acc : ",mse_new_state)
          #print("This is acc choosed : {0}".format(mse_new_state))
            
    def setSolution(self,solution):
        self.solution.set_state(solution)
        self.fitness = self.solution.get_mse(solution)
