...
class Swarm :
    ...
    def searchArea(self):
        ...
        h=0
        self.beeList=[]
        while((i<self.bees_number) and (i < self.flip) ) :# the search area is within flip so the solution given to bees are near refSol
            #print ("First method to generate") first method corresponds to fig2(a) referenced in BSO paper
            solution=self.refSolution.solution.get_state()
            k=0
            while((self.flip*k+h) < len(solution)): 
                solution[self.flip*k +h] = ((solution[self.flip*k+h]+1) % 2) #solution[flip*k+h] is flipped 
                k+=1
        ...
        while((i<self.bees_number) and (i< 2*self.flip )): # the search area is between flip and 2*flip,
            #so the solution given to bees are further to refSol
            #print("Second method to generate") second method corresponds to fig2(b) referenced in BSO paper
            ...
            i+=1
            h=h+1
        while (i<self.bees_number): # the rest bees are randomly generated
            ...
            self.beeList.append(newBee)
            i+=1
        # if solution is all zero i.e. subset is empty, re-initialize the solution using prob 0.8/0.2
    ...
    def selectRefSol(self):
      ...
      if(bestQuality<self.bestSolution.fitness):
          ...
          if(  (len(self.tabou)!=0) and  bestQuality < (self.tabou[-1].fitness)):#different from paper
              self.nbChance=self.max_chance
    ...
    def distanceTabou(self,bee):
        ...
        for i in range(len(self.tabou)):
            cpt=0 # number of different bit masks on same positions
    ...
    def bestBeeQuality(self):#intensification select the best solution as reference solution among Tabou
        distance = 0
        i=0
        pos=-1
        while(i<self.bees_number):
            min_val=self.beeList[i].fitness
            nbUn=Solution.nbrUn(self.beeList[i].solution.get_state())
            while((i<self.bees_number) and (self.beeList[i].solution.get_mse(self.beeList[i].solution.get_state()) == min_val)):
    ...
    def bestBeeDiversity(self): # return a solution which is furtherst from all the solutions stored in tabou. diversification
        max_val=0
    ...
    def bso(self,typeOfAlgo,flip):
        ...
        return self.bestSolution.fitness*100, Solution.nbrUn(self.bestSolution.solution.get_state()),self.bestSolution.solution.get_state()
      
      
    def count_features(self,solution):#number of being chosen after local search for every feature
        self.feature_count = {i:self.feature_count[i]+n for i, n in enumerate(solution)}

    def best_features(self):
        sorted_features = sorted(self.feature_count.items(), key=operator.itemgetter(1), reverse=True) # choose the most visited features
